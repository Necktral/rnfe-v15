...
class HomeoController:
    ...
    def __init__(self, config: Dict[str, Any], influx_logger=None):
        self.config = config
    ...
    def _switch_mode(self, new_mode: str):
        # Telemetría: log de cambio de modo
        try:
            self.influx_logger.log_event(
                name="homeostasis_mode_switch",
                tags={"from_mode": self.mode, "to_mode": new_mode}
            )
        except Exception as e:
    ...
    def health_status(self):
        """
        Cumple el protocolo HomeoProto. Devuelve el estado de salud actual.
        """
