...
class HierarchicalPredictiveCoder(nn.Module):
    def __init__(self, config):
        ...
        # Si existe: from aeon.core.module_orchestrator import LargeDenseModel
        try:
            from aeon.core.module_orchestrator import LargeDenseModel
            large_dense_model_available = True
        except ImportError:
            large_dense_model_available = False
        ...
        for i in range(self.cfg.num_layers):
            ...
            if i > 0:
                ...
                # Proyección: forzar arquitectura grande aunque las dimensiones sean menores
                if in_dim == 4096 and out_dim == 4096:
                    if large_dense_model_available:
                        self.attn_proj_layers.append(LargeDenseModel(4096, 4096))
                    else:
                        raise ImportError("LargeDenseModel no está disponible en aeon.core.module_orchestrator")
                else:
                ...
                # Para el primer layer, reemplazar nn.Identity por arquitectura grande (bridge)
                self.attention_mechanisms.append(
                    nn.Sequential(
                        nn.Linear(out_dim, 4096),
                        LargeDenseModel(4096, 4096),
                        nn.Linear(4096, 4096)
                    )
                )
